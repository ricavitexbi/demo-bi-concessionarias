...
def carregar_dados():
    ...
    try:
        df_inventario = pd.read_excel('/mnt/project/dados_concessionaria.xlsx', 
                                     sheet_name='Inventario_Atual')
        df_vendas = pd.read_excel('/mnt/project/dados_concessionaria.xlsx', 
                                 sheet_name='Historico_Vendas')
        df_clientes = pd.read_excel('/mnt/project/dados_concessionaria.xlsx', 
                                   sheet_name='Clientes')
        df_macro = pd.read_excel('/mnt/project/dados_concessionaria.xlsx', 
                                sheet_name='Dados_Macro')
        
        # Converter datas
        df_inventario['data_entrada'] = pd.to_datetime(df_inventario['data_entrada'])
        df_vendas['data_venda'] = pd.to_datetime(df_vendas['data_venda'])
        df_vendas['data_entrada_estoque'] = pd.to_datetime(df_vendas['data_entrada_estoque'])
...
if df_inventario is None:
    st.error("❌ Erro ao carregar dados. Verifique se o arquivo Excel existe no diretório correto.")
    st.stop()
...
if pagina == "📊 Dashboard Principal":
    ...
    with col4:
        ...
        criticos = len(df_inventario[df_inventario['status_estoque'] == 'Crítico'])
        st.metric(
            "⏱️ Tempo Médio Pátio",
    ...
    perf_conc.columns = ['Concessionária', 'Vendas', 'Receita', 'Lucro', 'Margem %']
    ...
    with col2:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color=[cores_status.get(s, 'gray') for s in status_estoque.index]
        ))
...
# ============================================================================
    ...
    # Treinar modelo
    X = modelo_df[['mes_numero', 'selic', 'confianca_consumidor']].fillna(method='ffill')
...
# ============================================================================
    ...
    if conc_selecionada != "Todas":
        df_filtrado = df_filtrado[df_filtrado['concessionaria_nome'] == conc_selecionada]
    if cat_selecionada != "Todas":
        df_filtrado = df_filtrado[df_filtrado['categoria'] == cat_selecionada]
    if status_selecionado != "Todos":
        df_filtrado = df_filtrado[df_filtrado['status_estoque'] == status_selecionado]
    ...
    with col2:
        st.metric("💰 Valor Total", f"R$ {df_filtrado['valor_compra'].sum()/1e6:.1f}M")
    with col3:
        st.metric("📅 Tempo Médio", f"{df_filtrado['dias_em_patio'].mean():.0f} dias")
    with col4:
        st.metric("💎 Ticket Médio", f"R$ {df_filtrado['valor_compra'].mean()/1e3:.0f}k")
    ...
    with col2:
        ...
        valor_cat = df_filtrado.groupby('categoria')['valor_compra'].sum().reset_index()
    ...
    df_display.columns = ['Modelo', 'Ano', 'Cor', 'KM', 'Dias Pátio', 'Preço', 'Status']
...
# ============================================================================
    ...
    with col2:
        ...
        margem_sugerida = ((preco_sugerido - valor_compra) / valor_compra) * 100
        ...
        **📈 Comparação com Mercado:**
        ...
        categorias = ['Valor Compra', 'Preço Atual', 'Preço Sugerido', 'Valor Fipe']
...
# ============================================================================
    ...
    with col2:
        st.metric("💰 Renda Média", f"R$ {df_clientes['renda_mensal'].mean():,.0f}")
    with col3:
        st.metric("🎂 Idade Média", f"{df_clientes['idade'].mean():.0f} anos")
    with col4:
        compradores = len(df_clientes[df_clientes['ja_comprou_antes'] == True])
        st.metric("🔄 Já Compraram", f"{compradores} ({compradores/len(df_clientes)*100:.0f}%)")
    ...
    categoria_preferida = df_clientes['preferencia_categoria'].value_counts().index[0]
    pct_financiado = (df_clientes['forma_pagamento_preferencial'] == 'Financiado').sum() / len(df_clientes) * 100
